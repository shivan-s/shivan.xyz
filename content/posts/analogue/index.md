---
title: Falling for Analogue
date: 2026-07-11
summary: Technology is great but it's okay to embrace the analogue
draft: false
cover:
  image: watch.jpg
  alt: An analogue watch on a person's wrist
  caption: A watch that just tells the time (feature creep - also tells day and date)
---

<script>
	import YouTube from "YouTube"
	import Figure from "Figure"
	import BlockQuote from "BlockQuote"
    import FountainPen from "./fountain-pen.jpg?enhanced"
    import Ink from "./ink.jpg?enhanced"
    import Watch from "./watch.jpg?enhanced"
    import Trmnl from "./trmnl.png?enhanced"
    import TrmnlDevice from "./trmnl-device.jpg?enhanced"
    import CarryJournal from "./carry-journal.jpg?enhanced"
    import SpencerianScript from "./spencerian-script.jpg?enhanced"
    import JellyStar0 from "./jelly-star-0.jpg?enhanced"
    import JellyStar1 from "./jelly-star-1.jpg?enhanced"
    import DeskAccessories from "./desk-accessories.jpg?enhanced"
</script>

This content was moved from my [`/now` page](/now).

---

When I started reading _Digital Minimalism_ by [Cal Newport](https://calnewport.com/), it's made me think about using technology to best serve me. The problem with technology such as smartphones is that they are both very useful and very distracting - all combined in one device.

So, there comes a point where I want to take some notes on my phone, but then I end up scrolling the web, wasting time and attention that could be used elsewhere.

Acknowleding this, I have purchased items inadvertenly appealing to an analogue lifestyle.

Let me go through those items...

## Fountain Pen

<Figure src={FountainPen} alt="A fountain pen illuminated by natural light; in the background is lined paper with handwritten text saying, 'I got a new fountain pen'" caption="My new fountain pen" />

The idea is that I want to journal more (in a handwritten journal), so maybe a better pen would help? A fountain pen is more expensive than regular pens - so I'm much less inclined to lose it. Also, regular pens, when they are all used up, get tossed away. With a fountain pen, you would fill it up with ink, so there is less waste.

<Figure src={Ink} alt="Bottle of ink called 'Jacques Herbin', subtitle 'Paris - 1670', and with a style 'Noir inspiration'" caption="Ink for my fountain pen" />

I got lovely scented ink too so that makes the process of writing even better. On top of that, fountain pens often involve almost no pressure when writing, improving the experience even further.

## Spencerian Script

Since I'm talking up writing with a fountain pen. I have decided to learn [Spencerial script](https://en.wikipedia.org/wiki/Spencerian_script).

<Figure src={SpencerianScript} alt="Collection of books that are related to spencerian script" caption="Mastering Penmanship" />

## Pocket Journal

I also got a small phone, which means it's difficult to take notes on. So, I've decided to carry around a small journal

<Figure src={CarryJournal} alt="Holding a small journal with a fountain tip pen" caption="Everyday carry journal" />

## Analogue Watch

The main purpose of a watch is to tell the time (any maybe day of the week and date for the month). However, now we have smart watches, which also tell the time but do more like monitor health biometrics, GPS, provide notifications and the list goes on.

These are too many features that provide minimal benefit, but a quite drastic detriments, which is constant connection. Newport in **Digital Minimalism** talks about two things: 1. solitude deprivation, where we don't disconnect complete from outside stimuli (e.g. constant connections and notifications); 2. marginal benefits, where smart watches do provide some benefits, like a computer on your wrist, but these benefits are outweighted by the first point and being without these small benefits maybe a minor nuisance but overall are better for you.

Cue the analogue watch.

<Figure src={Watch} alt="An analogue watch on a person's wrist" caption="Watch" />

## TRMNL

Not quite analogue, but e-Ink displays sure beat the bright screens we are accustomed to.

<Figure src={TrmnlDevice} alt="E-ink display showing github graph with 580 contributions in the last year" caption="An e-ink display" />

I purchased a [TRMNL e-ink display](https://trmnl.app/). This can be used to display words of the day, and quotes I've read in books, in place of notification on my phone.

The TRMNL company also have a cool "meet our team" section; it's sketchs of the team, plus links to their socials and websites.

<Figure src={Trmnl} alt="Character sketchs surrounding an architecture of a software system" caption="How cool is this?" />

## Bonus: New Mobile Phone

Not quite analoguing but I have a small smartphone, a Jelly Star from Unihertz.

<Figure src={JellyStar0} alt="Holding a small smartphone showing the front" caption="3 Inches is all you need" />

<Figure src={JellyStar1} alt="Holding a small smartphone showing the back" caption="Help in one hand" />

The idea is that the small formfactor makes it unappealing to scroll the web, but its still a full functioning smartphone, where I can make calls, browse the web, and use maps.

## Bonus: What's on my desk?

Here are some items on my desk to improve productivity, ergonomis and simply to create joy while work.

<Figure src={DeskAccessories} alt="An assortment of items related to a programmers desk with numbers overlayed to highlight each item" caption="Creating joy with small items" />

1. Split keyboard to help with ergonomics - [Moonlander Mark I](https://www.zsa.io/moonlander).
2. Ergonomic mouse that I use with Windows machines (I use the (3.) touchpad for MacOS) - Logitech MX Master 3S Bluetooth Edition.
3. Magic Trackpad that I use with MacOS.
4. Timer that shows a more physical representation of a countdown - [Time Timer](https://www.timetimer.com/)
5. E-ink Display - [Trmnl OG](https://trmnl.com/)
6. Pen Ink for my fountain pen
7. The morning ritual involves dark choclate paired with homemade espresso flat white. I usually have this in a [Ember cup](https://ember.com/) (not pictured) which keeps the brew warm
8. Misc. such as items to clean my desk as a mobility ball to roll out my wrist

## Conclusion

I'm starting to purchase items that are analogue and do one thing well in the effort to embrace digital minimalism.
