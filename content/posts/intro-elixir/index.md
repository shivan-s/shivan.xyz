...
In December 2025, I embarked on the programmer's annual tradition of partaking in [Advent of Code (AoC)](https://adventofcode.com/2025) after missing this for the last few years.
...
This year, however, I made the goal of learning a new language with AoC. And I also made it a point just to parse the files, not get all the stars.
...
It's built on top of [BEAM](<https://en.wikipedia.org/wiki/BEAM_(Erlang_virtual_machine)>) - which is the runtime for Erlang.
...
Most operations from code involve, taking data in, processing it, and sending it out.
...
["1", "2", "3", "4"]
# Or
> list |> Enum.map(fn n -> n |> Integer.to_string() end)
# Or even
> list |> Enum.map(&Integer.toString\\1)
```

### Function don't need `return`
...
This is achieved using Enum's `map`, `reduce`.
...
AoC with new languages touch the surface of what is possible in language. This was my first foray into a functional language involving immutability and that was my biggest takeaway. Other features include docs, `mix`, testing are useful too but didn't get a chance to look into.

Now when I use TypeScript, my primary language, I always think about immutability. I avoid mutability when I can and since I've used Elixir, I lean more into the functionality capabilities of JavaScript/TypeScript.
